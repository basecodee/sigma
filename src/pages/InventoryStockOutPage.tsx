...
import { TrendingDown, Plus, Package, Calendar } from 'lucide-react';
...
const InventoryStockOutPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (movementsResponse.status === 'success') {
        ...
        const outMovements = movementsResponse.data.filter((m: StockMovement) => m.movement_type === 'out');
        setMovements(outMovements);
      }
    } catch (err) {
  ...
  const handleAddStockOut = async (movementData: any) => {
    try {
      ...
      setError('Failed to add stock out');
    }
  };
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Kelola pengeluaran stok barang inventaris
            </p>
          </div>
          <button 
      ...
      <div className="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
        ...
        {movements.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-lg font-medium text-gray-900 dark:text-white">Belum ada stok keluar</p>
            <p className="text-sm text-gray-500 dark:text-gray-400">Tambahkan stok keluar untuk memulai tracking</p>
          </div>
        )}
      </div>
...
const StockOutModal: React.FC<StockOutModalProps> = ({ items, onSave, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (selectedItem && formData.quantity > selectedItem.stock_quantity) {
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4">
        ...
        <form onSubmit={handleSubmit} className="p-6">
          <div className="space-y-4">
            <div>
              ...
              {selectedItem && (
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                  Stok tersedia: {selectedItem.stock_quantity}
                </p>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                required
                min="1"
                max={selectedItem?.stock_quantity || 0}
          ...
          <div className="mt-6 flex justify-end space-x-3">
            ...
            <button
              type="submit"
              className="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-red-600 hover:bg-red-700 dark:bg-red-500 dark:hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors duration-200"
            >
              Keluarkan Stok
            </button>
          </div>
